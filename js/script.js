 ...
 * Handles navigation, galleries, video previews, and interactions
...
(function () {
    ...
    window.ImageProtection = {
        ...
        protectImage: img => {
            if (img && img.tagName === 'IMG') applyImageProtections(img);
        },
        unprotectImage: img => {
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        ...
                        if (node.querySelectorAll) {
                            node.querySelectorAll('.gallery-scroll').forEach(setupGalleryTouchHandlers);
                        }
                    }
                });
            });
    ...
    function enhanceImageClicks() {
        document.addEventListener('click', function (e) {
            ...
            if (imageItem && window.innerWidth <= 768) {
                ...
                if (img) {
                    if (typeof window.openZoomView === 'function') {
                        window.openZoomView(img.src, img.alt);
                    } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ===== STATE ===== */
    let currentSection = 'latest-work';
    let isMobileMenuOpen = false;
    let aboutGalleryInterval;

    /* ===== VIDEO SHOWCASE AUTOPLAY ===== */
    function initializeShowcaseVideos() {
        ...
        manageShowcaseVideoPlayback();
    }

    function manageShowcaseVideoPlayback() {
        const latestWorkSection = document.getElementById('latest-work');
        const isLatestWorkActive = latestWorkSection && latestWorkSection.classList.contains('active');

        showcaseVideos.forEach(video => {
            if (isLatestWorkActive && !document.hidden) {
                const playPromise = video.play();

                if (playPromise !== undefined) {
                    playPromise.catch(() => {
                        // Some browsers may block autoplay until interaction.
                        // Videos are muted, so most modern browsers should allow it.
                    });
                }
            } else {
                video.pause();
    ...
    document.addEventListener('visibilitychange', manageShowcaseVideoPlayback);
    ...
    if (packagesCta) {
        packagesCta.addEventListener('click', function () {
            const contactNavItem =
                document.querySelector('.nav-item[data-target="contact"]') ||
    ...
    function initializeMobileMenu() {
        ...
        document.addEventListener('touchend', e => {
            ...
            if (isMobileMenuOpen && dX > 50 && Math.abs(dY) < 50) {
                closeMobileMenu();
            }
        });
    }
    ...
    function initializeAboutGallery() {
        ...
        aboutGallery.addEventListener('touchstart', e => {
            touchStartX = e.touches[0].clientX;
        });
        ...
        aboutGallery.addEventListener('mouseenter', () => {
    ...
    function initializePortfolio() {
        ...
        initializeShowcaseVideos();
        showSection(currentSection);
        updateActiveNavItems(currentSection);
        manageShowcaseVideoPlayback();
